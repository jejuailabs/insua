...
/**
 * 감성엽서 문구 선정 (사용자 확정 사양).
 *
 * 사진이 있으면 **비전 모델이 사진을 직접 보고** 어울리는 한 줄을 고른다.
 * 사진이 없으면(=AI가 배경까지 만드는 경우) 엽서에 어울릴 장면과 문구를 함께 제안한다.
 *
 * 문구를 이미지 모델에게 "알아서 지어라"고 맡기지 않는 이유: 글자 수 규격(20~40자)과
 * 상투구 배제를 지킬 수 없고, 한글 렌더링도 정확한 문자열을 줘야 안정적이다.
 */
export async function pickPostcardCopy(photo?: {
  data: Buffer
  mime: string
}): Promise<{ quote: string; scene: string }> {
  const system = [
    '너는 감성 엽서를 만드는 20년차 인쇄물 디자이너다.',
    '사진(또는 만들 장면)에 어울리는 한국어 문구 한 줄을 고른다.',
    '',
    '규칙:',
    '1. 문구는 공백 포함 20~40자. 이 범위를 벗어나면 안 된다.',
    '2. 진부한 관용구·자기계발 상투구("오늘도 화이팅", "인생은 한 번뿐")를 쓰지 않는다.',
    '3. 이모지·따옴표·특수문자를 넣지 않는다. 문장부호는 마침표·쉼표까지만.',
    '4. scene 은 엽서 배경이 될 장면을 영어로 25자 내외로 묘사한다.',
    '',
    'JSON 만 출력한다. 키: quote, scene',
  ].join('\n')
  ...
  const content: Array<Record<string, unknown>> = [
    {
      ...
      text: photo
        ? '이 사진에 어울리는 문구를 골라라.'
        : '엽서로 만들기 좋은 잔잔한 장면 하나를 정하고, 거기 어울리는 문구를 골라라.',
    },
  ]
  if (photo) {
    content.push({
      type: 'image_url',
      image_url: { url: `data:${photo.mime};base64,${photo.data.toString('base64')}` },
    })
  }
  ...
  if (!res.ok) throw new Error(`POSTCARD_COPY_FAILED ${res.status}`)
  const json = await res.json()
  const raw = JSON.parse((json.choices?.[0]?.message?.content as string) ?? '{}')
  const quote = typeof raw.quote === 'string' ? raw.quote.trim() : ''
  return {
    // 규격을 벗어나면 안전한 기본 문구로 — 엽서가 아예 안 나오는 것보다 낫다
    quote: quote.length >= 12 && quote.length <= 48 ? quote : '오늘의 빛이 당신에게 오래 머물기를',
    scene: typeof raw.scene === 'string' ? raw.scene.trim() : 'a quiet seaside afternoon',
  }
}
