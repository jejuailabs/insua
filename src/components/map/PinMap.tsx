...
/**
 * 핀 지도 (사용자 확정 사양) — OSM 래스터 타일 + 자체 핀.
 * 외부 지도 라이브러리 없이 웹 메르카토르 투영을 직접 계산한다.
 * 히어로 지도와 CRM 고객 지도가 같은 물건이라 하나로 쓴다.
...
export function PinMap({
  ...
  function onPointerUp() {
    if (dragStart.current && drag) setCenter(pxToLatLng(c.x - drag.dx, c.y - drag.dy, zoom))
  ...
  return (
    <div
      className={cn(
        'relative touch-none overflow-hidden rounded-card border border-line bg-surface-2 select-none',
    ...
    >
      ...
      {pins.map((pin) => {
        const p = worldPx(pin.lat, pin.lng, zoom)
        return (
          <button
            key={pin.id}
            type="button"
            onClick={() => {
              if (!moved.current) onPinClick?.(pin.id)
            }}
