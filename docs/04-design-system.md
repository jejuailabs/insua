...
> **🔴 해결 필요 (M1)** — light 모드 3개 팔레트에서 **강조색 위 흰 텍스트가 3.0~3.4:1** 이다.
> 버튼 라벨이 13px 이라 "큰 텍스트 3:1" 예외를 받을 수 없다. 선택지는 둘이다.
>
> - (a) light 팔레트의 `--accent` 를 어둡게 조정한다 — 참조 이미지의 색감이 달라진다.
> - (b) light 모드에서만 `--text-on-accent` 를 어두운 색으로 바꾼다 — 버튼 인상이 달라진다.
>
> 어느 쪽이든 제품 인상이 바뀌는 **디자인 결정**이라 임의로 정하지 않았다. M1 착수 시 확인할 것.
>
