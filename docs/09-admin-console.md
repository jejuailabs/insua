...
#### 회원 삭제 = 완전 탈퇴 (소프트 삭제 원칙의 의도된 예외)

목적은 **같은 지메일로 처음처럼 다시 가입할 수 있게** 만드는 것이다.

Firestore 문서만 지우면 안 된다. **Firebase Auth 계정을 지워야** 다음 로그인에서
새 uid 가 발급되고 `/api/session` 이 users 문서를 새로 만든다. Auth 계정을 남긴 채
문서만 지우면 같은 uid 로 되살아나 "새 가입"이 아니다. 이전 역할 커스텀 클레임도 그대로 남는다.

`docs/02` §0 의 소프트 삭제 원칙(`deletedAt`)에 대한 **의도된 예외**다. 그 원칙은 신고 대응을 위해
콘텐츠를 남기려는 것이고, 이건 계정 자체를 없애는 탈퇴 처리다. 둘은 목적이 다르다.

동작 순서 (`adminDeleteUser`):

1. `users/{uid}` 를 **하위 컬렉션까지** 지운다(`recursiveDelete`). 찜 등이 유령으로 남지 않게.
2. Auth 계정 삭제. 이미 없으면(`auth/user-not-found`) 무시하고 진행한다.
3. `auditLogs` 에 `user.delete` 기록. 계정을 지워도 **누가 언제 지웠는지는 남는다.**
   `before` 에는 식별에 필요한 최소(이메일/역할)만 넣는다 (`docs/12` §6).

남은 데이터는 성격에 따라 셋으로 가른다. 한 덩어리로 처리하면 반드시 어딘가 틀린다.

| 성격 | 대상 | 처리 | 이유 |
|---|---|---|---|
| 개인정보 | `contacts`(+상담로그), `couponIssues`, `users/{uid}` 하위 | **삭제** | 소유자가 사라지면 Rules 상 아무도 못 읽는다. **읽지도 지우지도 못하는 PII** 를 남기는 게 가장 나쁜 결과다 |
| 공개 콘텐츠 | `posts` `products` `reviews` `boardPosts` `events` `stores` | **숨김** | 지우면 되돌릴 수 없다. 노출만 멈추고 판단은 콘텐츠 관리(§2.4)에 맡긴다 |
| 참조 | `anonymousPosts.authorUid`, `users.agentId` | **끊음** | 없는 계정을 가리키는 링크는 추적 리스크이자 오작동의 원인이다 |

세부 규칙:

- **매장은 `status: 'suspended'` 와 `isPublic: false` 를 둘 다** 내려야 실제로 안 보인다.
  다른 콘텐츠와 status 체계가 다르다.
- **후기를 숨길 때 매장 평점 집계를 함께 역산한다.** 안 하면 화면의 별점과 후기 수가 어긋난다.
  `adjustStoreReviewAggregate()` 를 콘텐츠 관리와 공유한다.
- **`users.agentId` 를 끊는 이유**: 안 끊으면 없는 설계사를 가리킨 채 AI 툴 게이트가
  계속 통과된다 (`docs/10` §5).
- **익명글은 남기고 `authorUid` 만 지운다.** 계정이 사라진 뒤의 역추적은 의미가 없고
  보관 자체가 리스크다 (`docs/02` §5). 본문과 `authorUidHash` 는 보존한다.
- `stores.createdByAgentId` 는 남긴다 — 누가 만들었는지의 이력이고, 지워도 얻는 게 없다.

제약:

- **자기 자신은 삭제할 수 없다.**
- **마지막 관리자는 삭제할 수 없다.** 관리자가 0명이 되면 아무도 되돌릴 수 없다.
  같은 이유로 **관리자 해제도 마지막 한 명에게는 막힌다** — 자기 자신만 막아서는
  다른 관리자가 마지막 한 명을 내리는 경우를 못 막는다.
  관리자 수는 `users.isAdmin`(미러)을 후보 목록으로만 쓰고 **커스텀 클레임으로 판정**한다.
  과다 집계(=덜 막음)가 과소 집계(=더 막음)보다 위험하기 때문이다.
- 확인 다이얼로그 없이는 실행되지 않는다. 삭제 버튼은 `--danger` 아웃라인,
  채운 배경은 확인창 안에서만 (§4).
- 처리 결과를 숫자로 돌려준다(`DeleteSummary`). 관리자가 무엇이 지워지고 무엇이
  숨겨졌는지 확인할 수 있어야 한다. 감사 로그의 `after` 에도 같은 요약이 들어간다.

**에뮬레이터 검증 25건 전부 통과** — 개인정보 삭제, 공개 콘텐츠 숨김·보존, 평점 집계 역산,
익명글 작성자 링크 제거, 담당 설계사 참조 해제, 같은 지메일 재가입 시 새 uid 발급,
마지막 관리자 삭제 거부, 본인 삭제 거부.
