...
- **자기 자신은 삭제할 수 없다.** 마지막 관리자가 스스로를 지워 시스템이 잠기는 걸 막는다.
- **작성한 콘텐츠는 함께 지우지 않는다.** 설계사의 고객카드까지 연쇄 삭제하면 되돌릴 수 없는
  범위가 너무 커진다. 남은 콘텐츠는 콘텐츠 관리(§2.4)에서 따로 처리한다.
  → 그 결과 **소유자가 사라진 데이터가 남을 수 있다.** `contacts` 는 Rules 가
  `ownerAgentId == uid` 를 요구하므로 아무도 읽을 수 없는 상태가 된다.
  설계사 계정을 지우기 전에 고객카드 이관·정리를 먼저 할지는 **운영 정책으로 결정 필요.**
- 확인 다이얼로그 없이는 실행되지 않는다. 삭제 버튼은 `--danger` 아웃라인, 채운 배경은
  확인창 안에서만 (§4).

에뮬레이터 검증: 삭제 후 같은 이메일로 재가입 → **새 uid 발급**, 이전 역할 클레임 없음,
`users` 문서·하위 `saved` 모두 사라짐, 감사 로그 1건 남음.
