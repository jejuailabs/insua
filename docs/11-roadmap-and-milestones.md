...
| M2       | 🟡   | 2026-08-08 | Rules 22건 통과, 라우트 가드 확인. 로컬 구글 로그인 OK. **프로덕션은 승인 도메인(`insua.vercel.app`) 콘솔 등록 대기** |
| M3       | ✅   | 2026-08-09 | Modal·TierBadge·SideRail·BottomTabBar·SectionHeader·HeroCarousel·ProductGrid. 로그인/역할선택을 모달로 전환, 메인=LOCAL HERO 공개 화면 |
| M4       | 🟡   | 2026-08-09 | contacts/interactions **Firestore 실데이터**. 신규 등록 폼(동의 3종 기본 해제)·검색·정렬·필터·단체문자(클립보드/CSV)·캘린더·상담로그·통계 동작. 남음: 사진 업로드, 지도 SDK, 녹음 실구현 |
| M5       | 🟡   | 2026-08-09 | 커뮤니티 홈 + 내 매장(모듈 수정·컴포저) + 공개 페이지 + **익명방 실데이터**(24h 작성자 수 실계산). 남음: 부동산·지원·공동구매 실데이터, 게시물 Storage 업로드 |
| M6       | 🟡   | 2026-08-09 | 피드(업종 연동 섹션)·찜(Firestore)·쿠폰 발급(couponIssues)·마이페이지(담당 설계사 지정+고지·동의)·히어로/마켓/이벤트. 남음: 반경 geohash 쿼리(권한 UX만 구현) |
| M7       | 🟡   | 2026-08-09 | 회원 목록·역할 변경(+auditLogs 기록)·감사 로그 뷰. 콘텐츠·신고는 빈 상태. 어드민 실검증은 bootstrap 계정 구글 로그인 후 |
| M8       | ⬜   |        |      |
